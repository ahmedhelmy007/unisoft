    ...
    public function deleteAction(Request $request, $id) {
        ...
        if (!$entity) {
         ...
         $querys = $repositorys->createQueryBuilder('p')               
                ->where('p.nationality = :id')
               // ->andWhere('p.nationality  IS NOT NULL')
         ...
         if($pro_entity){             
         $this->get('session')->getFlashBag()->add('notice2', 'Cannot Delete This Nationality Because Found relation Other table ');
         }else{
        $em->remove($entity);
        $em->flush();
