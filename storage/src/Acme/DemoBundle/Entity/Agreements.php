 ...
 * @ORM\Entity
 * @ORM\Entity(repositoryClass="Acme\DemoBundle\Entity\AgreementRepository")
 */
class Agreements {
     ...
     * @var string
     *
     * @ORM\Column(name="evacuation_type", type="string", length=45, nullable=true)
     */
    ...
    /**
     * @ORM\ManyToMany(targetEntity="Stores", inversedBy="agreement", cascade={"all"}))
     */
    private $store;
    ...
    private $agreementStatus;
     ...
     private $uniqeAgreement;
    
    /**
     * @ORM\OneToMany(targetEntity="ItemsDescription", mappedBy="agreement", cascade={"persist", "remove"})
     */
    protected  $itemsDescription;
    
    /**
     * @ORM\OneToMany(targetEntity="AuthorizedPersons", mappedBy="agreement", cascade={"persist", "remove"})
     */
    ...
    public function __construct()
    {
        ...
        $this->itemsDescription= new ArrayCollection();
    }
    ...
    public function getAgreementDate() {
        if (empty($this->agreementDate))
            $this->agreementDate = new \DateTime();
    ...
    public function getEndDate() {
      //month
        ...
        $date = strtotime("+".$this->getRentalPeriod()->getNumberOfUnits()." ".strtolower($this->getRentalPeriod()->getUnit()), $date);
//        $date->modify("+{$months} month");
        $this->endDate = new \DateTime(date('Y-m-d', $date));
        }
        return $this->endDate;
    }
    ...
    public function setItemsDescription($itemsDescription) {
        $this->itemsDescription = $itemsDescription;
    }

    public function getUniqeAgreement() {
        return $this->uniqeAgreement;
    }

    public function setUniqeAgreement($uniqeAgreement) {
        $this->uniqeAgreement = $uniqeAgreement;
    }
...
public function cloneStores($entities) {
    // Get current collection
    $stores = $this->getStore();
    $itemsDescriptions= $this->getItemsDescription();

    $this->store = new ArrayCollection();
    foreach ($stores as $store) {
        $clonestore = clone $store;
        $clonestore->getAgreement()->add($this);
    ...
    foreach ($entities as $item) {
        ...
        $cloneItem->setAgreement($this);
        $this->itemsDescription->add($cloneItem);
    }
    foreach ($itemsDescriptions as $item) {
        $cloneItem = clone $item;
        $cloneItem->setAgreement($this);
        $this->itemsDescription->add($cloneItem);
    }
